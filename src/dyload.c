...
int load_libpcap(void) {
	...
	if ((pcap_findalldevs = dlsym(libpcap, "pcap_findalldevs"), error = dlerror()) != NULL
		...
		|| (pcap_lookupnet = dlsym(libpcap, "pcap_lookupnet"), error = dlerror()) != NULL
		|| (pcap_setfilter = dlsym(libpcap, "pcap_setfilter"), error = dlerror()) != NULL
		|| (pcap_datalink = dlsym(libpcap, "pcap_datalink"), error = dlerror()) != NULL
...
int load_libnotify(void) {
	...
	if (!notify_init("evasi0n")) {
		printf("!! 初始化libnotify失败。\n");
		free_libnotify();
		return -1;
	}
	notify = notify_notification_new("802.1X Evasi0n", NULL, NULL, NULL);
	return 0;
}
